...
export default function DrinksInterface() {
  ...
  const filteredDrinks = drinks.filter((drink) => drink.category === selectedTab);
            ...
            <div className="flex items-center space-x-2 bg-gray-100 rounded-lg ">
              <button
                ...
                className={cn(
                  ...
                  viewType === "grid" ? "bg-white shadow-sm" : "hover:bg-white/50"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "p-2 rounded-md transition-colors",
                  viewType === "list" ? "bg-white shadow-sm" : "hover:bg-white/50"
                )}
              >
                ...
                <div className="grid grid-cols-4 gap-4">
                  {paginatedDrinks.map((drink, index) => (
                    <div key={index} className="flex flex-col items-center">
                      <Image src={drink.img} alt={drink.name} width={100} height={100} className="w-18 h-32 rounded-full mb-2" />
                      <p className="text-sm text-center">{drink.name}</p>
                      <p className="text-sm text-center text-gray-500">Qty: {drink.qty}</p>
                      <p className="text-sm text-center text-gray-500">GH¢ {drink.price.toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              ) : (
                    ...
                    {paginatedDrinks.map((drink, index) => (
                      <tr key={index} className="hover:bg-gray-100">
                        <td className="border border-gray-300 px-4 py-2">
                          <Image src={drink.img} alt={drink.name} width={64} height={64} className="w-18 h-24 rounded-full " />
                        </td>
                        ...
                        <td className="border border-gray-300 px-4 py-2 text-center text-gray-500">GH¢ {drink.price.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
